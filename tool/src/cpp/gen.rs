...
impl<'ccx, 'tcx: 'ccx> ItemGenContext<'ccx, 'tcx, '_> {
    ...
    pub(super) fn gen_cpp_to_c_for_type<'a, P: TyPosition>(
        ...
        match *ty {
            ...
            Type::Slice(Slice::Opaque(b, ref op)) => format!("{{reinterpret_cast<{}{}**>({cpp_name}.data()), {cpp_name}.size()}}",
                if b.mutability().is_mutable() { "" } else { "const " },
                self.formatter.namespace_c_name(op.id().into(), &self.formatter.fmt_type_name_unnamespaced(op.id()))
            ).into(),
    ...
    fn gen_c_to_cpp_for_type<'a, P: TyPosition>(
        ...
        match *ty {
            ...
            Type::Slice(hir::Slice::Opaque(b, ref op_ty)) => {
                let ty_name = self.formatter.fmt_type_name(op_ty.id());
                let ptr_name = format!("{ty_name}*");
                let span = self.formatter.fmt_borrowed_slice(&ptr_name, b.mutability());
                format!("{span}(reinterpret_cast<const {ty_name}**>({var_name}.data), {var_name}.len)").into()
            }
            Type::DiplomatOption(ref inner) => {
