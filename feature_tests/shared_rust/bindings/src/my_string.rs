...
impl MyString {
    ...
    pub fn new_from_utf16(v : &[&[u16]]) -> Box<MyString> {
        let ret = unsafe { MyString_new_from_utf16(v.iter().map(|u| { diplomat_runtime::DiplomatStr16Slice::from(*u) }).collect::<Vec<_>>().as_slice().into()) };
