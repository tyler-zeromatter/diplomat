...
impl<'a> Foo<'a> {
    ...
    pub fn get_bar<'a: 'b, 'b>(&'b self) -> Box<Bar<'b, 'a>> {
    ...
    pub fn extract_from_bounds<'a, 'x, 'y: 'a + 'x, 'z: 'y + 'a + 'x>(bounds : BorrowedFieldsWithBounds<'x, 'y, 'z>, another_string : &'a [u8]) -> Box<Foo<'a>> {
