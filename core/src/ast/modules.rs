...
pub fn parse_module_with_includes(module : &mut syn::ItemMod, base_path : &std::path::Path) -> Result<(), std::io::Error> {
    ...
    if contains_bridge {
        ...
        let (_, items) = module.content.as_mut().expect("Expected mod content in diplomat::bridge.");
        for i in attrs.includes {
            let file_contents = std::fs::read_to_string(base_path.join(i.path))?;
            let syn_file = syn::parse_file(&file_contents).map_err(|e| {
    ...
    if let Some((_, items)) = module.content.as_mut() {
        for i in items {
            match i {
                Item::Mod(m) => parse_module_with_includes(m, base_path)?,
                _ => {}
            }
        }
    }
...
pub fn parse_file_with_includes(mut file : syn::File, base_path : &std::path::Path) -> Result<syn::File, std::io::Error> {
    for i in file.items.iter_mut() {
        match i {
            syn::Item::Mod(m) => {
                parse_module_with_includes(m, base_path)?;
            }
            _ => {}
        }
    }
